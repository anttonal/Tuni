...
public:
     ...
     * @param params vector containing parameters of the corresponding command
     * Prints the name of each staff member
     */
...
private:
    ...
    /**
      * @brief Person_Ptr_Comparator
      * @param operator() Allows the comparison of two
      * person pointers in a container like std::set
      */
    struct Person_Ptr_Comparator
    {
    ...
    // Patients who haven't left the hospital after entering
    std::map<std::string, Person*> current_patients_;

    // Staff members recruited
    std::map<std::string, Person*> staff_;

    // All patients that have entered at some point
    std::map<std::string, Person*> all_patients_;

    // Map of a patients name as a key and all theircare periods as a value
    std::map<std::string, std::set<Care_Period*>> patient_care_periods_;

    // Map of a staff members name as a key and all
    // care periods they are assigned to as a value
    std::map<std::string, std::set<Care_Period*>> staff_care_periods_;

    // Map of medicine names as a key and all patients
    // that have had that medicine assigned to them´
    std::map<std::string, std::set<Person*, Person_Ptr_Comparator>> assigned_medicine_;
};
