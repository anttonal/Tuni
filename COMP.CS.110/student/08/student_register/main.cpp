...
void change_number(const std::string& target_number,
                   std::map< std::string, Student* >& student_numbers,
                   std::map< std::string, Student* >& user_ids, const std::string& file_name){

    if ( student_numbers.find(target_number) == student_numbers.end() ){
    ...
    if ( is_valid_phone_number(new_number)){
        for ( auto& student_number : student_numbers ){
            if ( student_number.first == target_number ){
                student_number.second->phone_number = new_number;
            }
        }
        std::ofstream file_object(file_name);
        for ( auto& student_number : user_ids ){
            file_object << student_number.second->student_number + ";"
                        << student_number.second->user_id + ";"
                        << student_number.second->name + ";"
                        << student_number.second->phone_number + ";"
                        << student_number.second->email + ";"
                        << student_number.second->skype << std::endl;
        }
        file_object.close();
    }
}
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            change_number(parts.at(1), student_numbers, user_ids, file_name);
